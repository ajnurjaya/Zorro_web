<?php 
    ...
    $DbName             = "dbname";
    $backup_name        = "join_dbhuawei_4G.sql";
    $filename           = 'mybackup_dbhuawei_4G.sql';
    $filename2          = 'join_dbhuawei_4G.sql';
    $filename3          = 'mybackup_dbeid_4G.sql';
    ...
    echo "import table pertama eid berhasil";
    mysqli_close($con1);
    ...
    $query2 = mysqli_query($con2,"SELECT * FROM huawei_zp_4g as t1 LEFT JOIN site_list as t2 ON t1.site_id=t2.site_id WHERE DATE = '$today' AND hour = '$hour'");
    ...
    for ($i = 0, $st_counter = 0; $i < $fields_amount;   $i++, $st_counter=0)
    {
       while($row = mysqli_fetch_row($query2))
       {
           if ($st_counter%100 == 0 || $st_counter == 0 )  
                    {
                            $content .= "\nINSERT INTO zpzt_4g_huawei VALUES";
                    }
                    $content .= "\n(";
    ...
    echo $content;
    $fp = fopen('join_dbhuawei_4G.sql', 'w');
    fwrite($fp, $content);
    fclose($fp);
    mysqli_close($con2);
